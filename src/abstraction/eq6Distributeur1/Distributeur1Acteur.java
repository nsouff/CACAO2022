		...
		this.getNotreStock().getMapStock().forEach((key,value)->{
			if (value <= 10000) {
				...
				if (ListeVendeurs.size() != 0) {
					...
					ExemplaireContratCadre CC = supCCadre.demandeAcheteur((IAcheteurContratCadre)this,Vendeur, value, new Echeancier(Filiere.LA_FILIERE.getEtape()+1,12,20000), cryptogramme, false);
					if (CC == null) {
