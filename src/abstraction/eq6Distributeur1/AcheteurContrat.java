...
public class AcheteurContrat extends DistributeurChocolatDeMarque implements IAcheteurContratCadre{//leorouppert
	...
	public Map<ChocolatDeMarque, Echeancier> nouveauxEcheanciersVoulus() {
		...
		for (ChocolatDeMarque choco : Filiere.LA_FILIERE.getChocolatsProduits()) {
			...
			if (e.getQuantiteTotale() > SEUIL_AJOUT_ECHEANCE*attenduNProchainesEtapes(24, choco)*partDuMarcheVoulu(choco.getChocolat())*partCC) {// Sinon cela ne sert à rien de faire un nouveau contrat cadre
				res.put(choco, e);
			}
		}
		return res;
