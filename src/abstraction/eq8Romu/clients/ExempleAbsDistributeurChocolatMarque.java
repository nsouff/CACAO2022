...
public class ExempleAbsDistributeurChocolatMarque implements IActeur {
	...
	public ExempleAbsDistributeurChocolatMarque(ChocolatDeMarque[] chocos, double[] stocks) {	
		...
		for (int i=0; i<chocos.length; i++) {
			...
			this.stocksChocolats.put(chocos[i], new Variable(this.getNom()+"Stock"+chocos[i].name(), this, 0.0, 1000000000.0,stocks[i]));
		}
	...
	public void next() {
		...
		if (Filiere.LA_FILIERE.getEtape()>=1) {
			...
			journal.ajouter("Le prix moyen du chocolat \""+chocolats.get(i).name()+"\" a l'etape precedente etait de "+Filiere.LA_FILIERE.prixMoyen(chocolats.get(i), Filiere.LA_FILIERE.getEtape()-1));
