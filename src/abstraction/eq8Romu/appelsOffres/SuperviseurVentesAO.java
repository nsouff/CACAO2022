 ...
 * @param quantiteKG doit etre superieur a OffreVente.AO_QUANTITE_MIN
 ...
 * @return
 */
	public PropositionAchatAO vendreParAO(IVendeurAO vendeur, int cryptogrammeVendeur, ChocolatDeMarque choco, double quantiteKG, boolean enTG) {
		if (vendeur==null) {
		...
		if (quantiteKG<OffreVente.AO_QUANTITE_MIN) {
			throw new IllegalArgumentException(" appel de vendre(...) de SuperviseurVentesAppelOffre avec "+quantiteKG+" pour quantite (min="+OffreVente.AO_QUANTITE_MIN+"))");
		}
		OffreVente offre = new OffreVente(vendeur, choco, quantiteKG, enTG);
		journal.ajouter(Journal.texteColore(vendeur, vendeur.getNom())+" veut vendre "+Journal.doubleSur(quantiteKG, 2)+ " de "+choco);
		...
		if (acheteurs.size()>0) {
			for (IAcheteurAO a : acheteurs) {
				...
				if (prix<=0.0) {
					...
					if (this.laBanque.verifierCapacitePaiement((IActeur)a, cryptos.get((IActeur)a), prix*quantiteKG)) {
			...
			if (propositions.size()>0) {
				...
				if (retenue != null) {
					...
					this.laBanque.virer(retenue.getAcheteur(), this.cryptos.get(retenue.getAcheteur()), vendeur, retenue.getPrixKg()*retenue.getOffre().getQuantiteKG());
