...
public class Producteur2VendeurContratCadre extends Producteur2Acteur implements IVendeurContratCadre{
	...
	public void next() {
		...
		this.journalCC.ajouter("Quantité par step de Feve HAUTE Non Bio : "+this.quantiteTotaleContratEnCours(Feve.FEVE_HAUTE));
		this.journalCC.ajouter("Quantité par step de Feve MOYENNE Non BIO  : "+this.quantiteTotaleContratEnCours(Feve.FEVE_MOYENNE));
		this.journalCC.ajouter("Quantité par step de Feve BASSE Non BIO  : "+this.quantiteTotaleContratEnCours(Feve.FEVE_BASSE));
...
//	@Override
	...
	public double propositionPrixBio(ExemplaireContratCadre contrat) {
		BourseCacao bourse = (BourseCacao)(Filiere.LA_FILIERE.getActeur("BourseCacao"));
		double prix = 1.25*bourse.getCours((Feve)(contrat.getProduit())).getValeur();
		if (this.getClassementTransformateur( contrat.getAcheteur() )==1){
			prix = 1.15*bourse.getCours((Feve)(contrat.getProduit())).getValeur();
		}
